...
void ExpressionCompiler::appendConstStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	// append return
	m_context << m_context.dupInstruction(_varDecl.annotation().type->sizeOnStack() + 1);
...
void ExpressionCompiler::appendStateVariableAccessor(VariableDeclaration const& _varDecl)
{
	...
	{
		m_context << m_context.swapInstruction(static_cast<unsigned>(paramTypes.size()));
		m_context << Instruction::POP;
		m_context << m_context.swapInstruction(static_cast<unsigned>(paramTypes.size()));
	...
	m_context << m_context.dupInstruction(retSizeOnStack + 1);
	m_context.appendJump(evmasm::AssemblyItem::JumpType::OutOfFunction);
}
	...
	else  // compound assignment
	{
		...
		if (lvalueSize > 0)
		{
			...
			for (unsigned i = 0; i < itemSize; ++i)
				m_context << m_context.swapInstruction(itemSize + lvalueSize) << Instruction::POP;
		}
		...
		if (!_unaryOperation.isPrefixOperation())
		{
			...
			if (m_currentLValue->sizeOnStack() > 0)
				for (unsigned i = 1 + m_currentLValue->sizeOnStack(); i > 0; --i)
					m_context << m_context.swapInstruction(i);
		}
...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
		...
		case FunctionType::Kind::Creation:
		{
			...
			if (function.saltSet())
			{
				m_context << m_context.dupInstruction(2 + (function.valueSet() ? 1 : 0));
			...
			if (function.valueSet())
				m_context << m_context.dupInstruction(3 + (function.saltSet() ? 1 : 0));
			else
				m_context << u256(0);
			...
			if (function.valueSet())
				m_context << m_context.swapInstruction(1) << Instruction::POP;
			if (function.saltSet())
				m_context << m_context.swapInstruction(1) << Instruction::POP;
		...
		case FunctionType::Kind::SetGas:
		{
			...
			if (stackDepth > 0)
				m_context << m_context.swapInstruction(stackDepth);
			if (function.gasSet())
				m_context << Instruction::POP;
			break;
		}
...
)
{
	...
	{
		m_context << m_context.dupInstruction(2 + gasValueSize + CompilerUtils::sizeOnStack(argumentTypes));
	...
	if (isDelegateCall)
		...
		m_context << m_context.dupInstruction(m_context.baseToCurrentStackOffset(valueStackPos));
	else
		m_context << u256(0);
	m_context << m_context.dupInstruction(m_context.baseToCurrentStackOffset(contractStackPos));
	...
	if (_functionType.gasSet())
		m_context << m_context.dupInstruction(m_context.baseToCurrentStackOffset(gasStackPos));
	...
	if (!returnSuccessConditionAndReturndata && !_tryCall)
	{
		...
		m_context << m_context.swapInstruction(remainsSize);
