...
void ArrayUtils::copyArrayToMemory(ArrayType const& _sourceType, bool _padToWordBoundaries) const
{
	...
	else
	{
		...
		m_context << Instruction::DUP1 << m_context.dupInstruction(haveByteOffset ? 5 : 4);
...
void ArrayUtils::retrieveLength(ArrayType const& _arrayType, unsigned _stackDepth) const
{
	...
	{
		m_context << m_context.dupInstruction(1 + _stackDepth);
...
void ArrayUtils::incrementByteOffset(unsigned _byteSize, unsigned _byteOffsetPosition, unsigned _storageOffsetPosition) const
{
	...
	if (_byteOffsetPosition > 1)
		m_context << m_context.swapInstruction(_byteOffsetPosition - 1);
	m_context << u256(_byteSize) << Instruction::ADD;
	if (_byteOffsetPosition > 1)
		m_context << m_context.swapInstruction(_byteOffsetPosition - 1);
	// compute, X := (byteOffset + byteSize - 1) / 32, should be 1 iff byteOffset + bytesize > 32
	m_context
		<< u256(32) << m_context.dupInstruction(1 + _byteOffsetPosition) << u256(_byteSize - 1)
		<< Instruction::ADD << Instruction::DIV;
	// increment storage offset if X == 1 (just add X to it)
	// stack: X
	m_context
		<< m_context.swapInstruction(_storageOffsetPosition) << m_context.dupInstruction(_storageOffsetPosition + 1)
		<< Instruction::ADD << m_context.swapInstruction(_storageOffsetPosition);
	// stack: X
	...
	if (_byteOffsetPosition == 1)
		...
		m_context
			<< m_context.dupInstruction(_byteOffsetPosition + 1) << Instruction::MUL
			<< m_context.swapInstruction(_byteOffsetPosition) << Instruction::POP;
}
