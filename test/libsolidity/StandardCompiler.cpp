	...
	static std::vector<std::tuple<Json, std::optional<std::function<bool(Json)>>>> tests{
		...
		},
		{
			...
			[](const Json& result)
			{
				...
				auto const emptyOutput = [&](std::string const& _kind) {
					return !evm.contains(_kind) || !evm[_kind].contains("ethdebug") || evm[_kind]["ethdebug"].is_null();
				};
				return
					result.dump().find("/// ethdebug: enabled") == std::string::npos &&
					emptyOutput("bytecode") && emptyOutput("deployedBytecode");
			}
		},
		{
...
BOOST_AUTO_TEST_CASE(ethdebug_ethdebug_output)
{
	static std::vector<std::tuple<Json, std::optional<std::function<bool(Json)>>>> tests{
		...
		// info selection implicitly; the output is empty without it.
		{
			generateExperimentalStandardJson(true, {}, Json::array({"evm.bytecode.ethdebug"})),
			[](const Json& result)
			{
				return result["contracts"]["fileA"]["C"]["evm"]["bytecode"]["ethdebug"].is_null();
			}
		},
		{
