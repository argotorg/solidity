...
public:
	...
	size_t numInstructions(bytes const& _bytecode, std::optional<Instruction> _which = std::optional<Instruction>{})
	{
		...
		evmasm::eachInstruction(realCode, m_evmVersion, [&](Instruction _instr, u256 const&, int) {
...
BOOST_AUTO_TEST_CASE(retain_information_in_branches)
{
	...
	size_t numSHA3s = 0;
	eachInstruction(optimizedBytecode, m_evmVersion, [&](Instruction _instr, u256 const&, int) {
