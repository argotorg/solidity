		...
		for (auto const& [instId, trace]: ranges::views::zip(block.instructions, blockLayout->operationShuffles))
		{
			...
			if (!inst.isOperation())
			{
				if (!trace.empty())
				{
					...
					if (inst.isPhi())
						_out << slotToString(StackSlot::makeValue(m_cfg, instId)) << " := " << slotToString(StackSlot::makeShadow(instId));
					else if (inst.isUpsilon())
						_out << slotToString(StackSlot::makeShadow(m_cfg.upsilonPhi(instId))) << " := " << slotToString(StackSlot::makeValue(m_cfg, inst.inputs.front()));
					else
						_out << slotToString(StackSlot::makeValue(m_cfg, instId)) << " := " << slotToString(StackSlot::makeValue(m_cfg, inst.inputs.front()));
					_out << "\\l\\\n";
					_out << stackToString(operationStack) << "\\l\\\n";
				}
				continue;
