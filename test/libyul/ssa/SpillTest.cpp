...
)
{
	...
	for (auto const& [key, address]: _spilled)
		out += fmt::format(
			"    {} ({}) -> mem {}\n",
			key,
			key.isShadow() ? "shadow" : key.isPhiValue() ? "phi" : "value",
	...
	for (const auto& key: _spilled | std::views::keys)
	{
		if (key.isShadow())
		{
			// a shadow is stored at each of its upsilons
			for (SSACFG::BlockId const blockId: _cfg.liveBlocks())
				_cfg.forEachUpsilon(_cfg.block(blockId), [&](InstId const upsilonId, SSACFG::Inst const& upsilon) {
