	...
	std::vector<SpillStorePlan::Store>& _stores)
{
	...
	{
		std::size_t const erased = planningSpills.m_values.erase(value);
	...
	{
		if (!stackIt->isValue() || !uninitializedPending.contains(stackIt->value()))
			continue;
		InstId const value = stackIt->value();
		...
		std::size_t const spillCountBefore = planningSpills.numSpilled();
		...
		// The shuffler only adds spills, so an unchanged count means there is nothing new to merge.
		if (planningSpills.numSpilled() != spillCountBefore)
	...
	yulAssert(uninitializedPending.empty(), "same-site spill is absent from its definition stack");
	return true;
}
