...
/// stack top and concludes with the `Store` op consuming it. Keyed by the defining Inst: the value itself for an
/// SSA value, the upsilon for a phi shadow (which has one def site per upsilon).
using SpillStoreTraces = std::map<InstId, ShuffleTrace>;

/// Per-CFG set of variables (SSA values and phi shadows) spilled to memory
class SpillSet
{
public:
