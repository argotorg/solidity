...
)
{
	...
	while (true)
	{
		...
		spillSet.closeUnderReachabilityConstraints(_liveness.cfg(), generator.m_resultLayout, spillStorePlan);
		// only the plan of the final iteration survives; it is recorded against the stable spill set
		if (spillSet.numSpilled() == spillCountBefore)
			return Result{std::move(generator.m_resultLayout), std::move(spillSet), std::move(spillStorePlan)};
...
void StackLayoutGenerator::visitBlock(SSACFG::BlockId const& _blockId)
{
	...
	m_cfg.forEachOperation(block, [&](InstId const _instId, SSACFG::Inst const& _inst) {
		...
		for (std::size_t i = 0; i < requiredStackTop.size(); ++i)
			stack.pop();
		// The shuffle may have parked a copy of the call's return label in a junk position of the tail. After the
		// call it is dead data, so record it as junk; otherwise a successor's stack-in could demand the label.
		for (auto& slot: currentStackData)
			if (slot.isFunctionCallReturnLabel())
				slot = Slot::makeJunk();
